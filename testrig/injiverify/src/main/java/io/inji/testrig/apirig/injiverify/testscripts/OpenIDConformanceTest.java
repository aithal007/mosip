...
/**
 * Runs the OpenID Foundation OID4VP verifier test plan against the Inji Verify instance this
 * testrig targets ({@code injiVerifyBaseUrl}) and reports one TestNG result per conformance
 * module in the regular EmailableReport.
 *
 ...
 * ignored.
 */
public class OpenIDConformanceTest extends InjiVerifyUtil implements ITest {

	private static final Logger logger = Logger.getLogger(OpenIDConformanceTest.class);
	private static final String COMPONENT = "verify";
	private static final String PREFIX = "InjiVerify";
	...
	public static void setLogLevel() {
		logger.setLevel(InjiVerifyConfigManager.IsDebugEnabled() ? Level.ALL : Level.ERROR);
	}
	...
	public Object[] getTestCaseList(ITestContext context) {
		ConformanceRunConfig config = ConformanceRunConfig.fromLookup(COMPONENT, injiVerifyBaseUrl + "/v1/verify",
				InjiVerifyConfigManager::getproperty);
	...
	public void test(TestCaseDTO testCaseDTO) throws AdminTestException {
		...
		ConformanceResults.Module module = results.find(testCaseDTO.getInput())
				.orElseThrow(() -> new AdminTestException("Conformance module missing from results: " + testCaseDTO.getInput()));
